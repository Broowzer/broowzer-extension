...
import {
  ...
  ToggleFlex,
  ToggleGrid
} from 'broowzer-ui/features/shields'
...
export default class ShieldsInterfaceControls extends React.PureComponent<Props, State> {
  ...
  render () {
    const { broowzerShields, url, hostname, ads, trackers, httpsRedirected, httpUpgradableResources } = this.props
    const { openTotalAdsTrackersBlockedList, openConnectionsEncryptedList } = this.state

    const enabled = broowzerShields !== 'block'
    ...
    return (
      <div id='broowzerShieldsInterfaceControls'>
        {/* ads toggle */}
        <ToggleGrid>
